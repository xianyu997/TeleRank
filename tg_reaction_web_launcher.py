...
PREFERRED_PORT = 1717
APP_VERSION = "2026-07-11-local-import-progress"
PREFS_FILE = Path.home() / "TelegramReactionRanker" / "preferences.json"
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_pick_folder(self):
        if not self.is_local_client():
            ...
            return
        if sys.platform != "darwin":
            self.send_json({"ok": False, "error": "folder picker only available on macOS"}, 400)
            return
