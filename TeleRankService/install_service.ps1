...
if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    $argList = "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`" -Port $Port -DataDir `"$DataDir`" -ArchiveRoot `"$ArchiveRoot`" -TrashRoot `"$TrashRoot`" -Proxy `"$Proxy`""
...
New-ItemProperty -Path $regPath -Name Proxy -Value $Proxy -PropertyType String -Force | Out-Null
